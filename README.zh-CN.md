...
| Pin 方式 | 自动升级 | 可复现性 | 适用场景 |
|---------|---------|---------|---------|
| `@v2`（滑动主版本） | 是，每次 CI 运行 | 中 | **默认推荐。** 希望自动收到安全修复与新语言支持，而不需要开 PR。 |
| `@v2.2.1`（精确 tag） | 否 | 高 | 受监管/审计环境，不允许静默的依赖升级。 |
| `@<40 位 SHA>`（提交哈希） | 否 | 最高 | 最严格的供应链姿态；只有你手动改 ci.yml 时才会变。 |

`@v2` 是文档推荐值，也是安装脚本默认写入的值。完整生命周期（紧急回滚、老主版本支持期）见 [版本与升级策略](#版本与升级策略)。
...
|--------|----------|--------------|------|-----------|
| Node.js / TypeScript | `package.json` | 根据锁文件自动识别 `npm` / `pnpm` / `yarn`，然后 install → lint → build | 若存在 `test` 脚本则执行 | `coverage/` → Codecov |
...
| [Checkov](https://www.checkov.io/) | IaC（Terraform / Kubernetes / Dockerfile / Secrets / Actions） | 可选硬门禁 + SARIF |
| [REUSE](https://reuse.software/) | SPDX 许可证合规 | 可选 |
| [OpenSSF Scorecard](https://scorecard.dev/) | 每周行业安全评分 | 徽章 + 工作流产物 |
...
jobs:
  ci:
    ...
    #   enable-checkov: true        # IaC 策略门禁；发现问题即失败
    # secrets:
...
| `enable-checkov` | boolean | `true` | 在检测到 IaC 文件时运行 Checkov，并在发现问题时让 CI 失败。 |
...
每个工作流顶层声明 `permissions: contents: read`，必要时才在 job 级别细粒度赋权（例如 CodeQL、Checkov 上传 SARIF 时的 `security-events: write`）。
...
当**这个模板仓库本身**收到一次推送到 `main` 时，GitHub Actions 会围绕“可复用流水线”再跑一层维护型工作流：

```mermaid
flowchart TB
    A([ci-templates 收到 push 到 main]) --> B[CI - YAML Lint]
    A --> C[zizmor]
    A --> D[Gitleaks]
...
你可以把它理解为**模板仓库自己的质量门禁**：

- `CI - YAML Lint`：确保工作流 YAML 没写坏，避免下游用户拿到不可运行的模板。
- `zizmor (Actions SAST)`：检查 GitHub Actions 是否存在不安全写法，例如危险表达式、权限过大、问题 action 用法等。
- `Gitleaks (secret scan)`：扫描完整 git 历史，防止误提交 Token、私钥、密码等敏感信息。
- `OpenSSF Scorecard`：从仓库治理层面持续评估安全成熟度，例如分支保护、依赖更新、权限最小化等。
- `CodeQL`：对 `actions` 语言做静态分析，发现工作流注入与逻辑风险。
- `Release Please`：自动管理 release PR、changelog、GitHub Release，以及下游依赖的滑动 `v2` 标签。
- `Deploy landing page to GitHub Pages`：重新部署 `docs/`，让公开落地页与 README / starter 文件保持一致。

这套流程与 `reusable-ci.yml` 中的**下游项目流水线**是两回事：下游仓库运行的是可复用 CI；而本仓库除了发布模板，还要验证模板本身、治理本身、文档本身。
